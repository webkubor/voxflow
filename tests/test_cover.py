...
**比例校验**：写死枚举会把上游的能力阉掉一半 —— 中台的 gpt-image-2 支持任意
`W:H`（含小数比例如 1:2.1），CLI 帮助里那个五选一只是常用值提示。这里断言的是
「常用值之外的照样放行、写错的挡住」，防的是有人图省事改回白名单。

**画幅核对**：上游**不一定按请求的比例出图**。2026-09-05 实测 tronzen 收到
800x1680（1:2.1）返回 1680x1680 方图，而同期 jizhi / xinhankr 收到 1792x1008
返回 1791x1007 / 1792x1009（遵守）。中台的 image-size.js 自己写着「用户拿到
比例不对的图而不知情，比直接报错糟糕得多」—— 这个核对就是为了不让它悄悄发生。

难点在容差：太严会把 1~2px 的正常取整报成不符，太松会放过 tronzen 那种
把长图出成方图的。下面用两组真实实测数据把这条线钉住。
"""
...
def test_ratio_match_tolerates_rounding() -> None:
    ...
    这两组是 2026-09-05 的真实实测值。
    """
    check("jizhi 1791x1007 算 16:9", _ratio_matches("16:9", (1791, 1007)) is True)
    check("xinhankr 1792x1009 算 16:9", _ratio_matches("16:9", (1792, 1009)) is True)
    check("1254x1254 算 1:1", _ratio_matches("1:1", (1254, 1254)) is True)


def test_ratio_match_catches_silent_squaring() -> None:
    """
    tronzen 把 1:2.1 出成了 1680x1680 方图。这种必须抓住 ——
