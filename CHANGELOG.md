...
- **`core/cover.py`**：`POST /api/generate` → 轮询 `/api/jobs` → 下载 → 回填台账
  的 `cover_file`。走 HTTP 直调而不是 museav CLI —— CLI 的 README 明确说产品集成
  走 HTTP 或把 StudioClient 当库导入，CLI 只服务「人在终端 / agent 跑 shell」。
- **计费：一张 1 积分 ≈ ¥0.83**（真源：museav-manager 的 `shared/credits-catalog.js`，
  标准档 1 分 / hd 档 2 分）。`configs/pricing.json` 里原来那个 0.05 元/积分的
  占位值低估了 16 倍，已按积分包定价（Pro Pack $69/600）修正为 0.83。
- **不要传 `quality: "high"`**。一开始传了，**纯亏一倍**。对照实验：
  ...
  同样尺寸体积，扣费差一倍。三条原因：`quality` 是 gpt-image 系的私有**画质**
  参数、不控制尺寸；gen-worker 不传时本来就自动补 high；`providers.js` 明写
  「有带画质参数的也有不带的，单张价基本纹丝不动」。这个默认值还漏改过一处 ——
  改了 `cover.generate()` 却没改 `CoverRequest`，**一个默认值分散在两处，
  只改一处就是这种下场**。
- **比例完全可配，不限枚举**。`ratio` 支持任意 `W:H`（含小数比例如 1:2.1）——
  中台的 gpt-image-2 本来就能传任意尺寸，CLI 帮助里那个五选一只是常用值提示，
  写死枚举等于把上游能力阉掉一半。看板顶栏加了比例输入框（用 `<input list>`
  而不是 `<select>`，正是为了不锁死在几个枚举上），选择记进 localStorage。
  格式写错在提交时 400 挡掉，不丢进任务队列再失败。
- **出图后核对实际画幅**。上游不一定按请求的比例出图 —— 实测 tronzen 收到
  800×1680（1:2.1）返回 **1680×1680 方图**，而 jizhi / xinhankr 收到 1792×1008
  返回 1791×1007 / 1792×1009（遵守）。容差 5%：放过 1~2px 的正常取整，抓住把
  长图出成方图的。不符时结果带 ⚠️ 提示并记一条 warn 日志 —— 图是好图只是画幅
  不对，不重试也不报错，让人自己决定要不要换上游重出。`null`（量不出来）和
  `false`（确认不符）严格分开。
- **额度：自家租户不该被闸门卡**。voxcraft 余额 0 却被 403 拦住，根因在中台
  `deduct_tenant_credit` 缺自家租户豁免 —— 已在 museav-manager 提交
  `20260906090000_platform_tenant_unmetered` 修掉（含退款侧对称处理，否则失败
  退款会让余额凭空增长），`/api/balance` 新增 `unmetered` 字段。VoxFlow 据此
  判断而不再只看余额：自家租户余额恒为 0 而出图正常，只看余额会一直把按钮灰着。
- **尺寸达标了**：`1:1 → 1440x1440`（汽水要 ≥1440、网易云要 ≥1400，一张图两边都够）。
  原因不是「换个上游」能解决的 —— 是 `ratioToSize` 把像素预算写死在 1,572,864，
  而 gpt-image-2 的上限是 8,294,400，**只用了 19%**。中台加了 `size` 入参
  （而不是放大默认 prefer：后者会改变所有租户的出图成本，那是定价决策），
  VoxFlow 的 `_size_for()` 按比例算短边 ≥1440 且两边都是 16 倍数的尺寸；
  比例太极端算不出来时回落到只传 ratio —— 宁可拿张小的，也别出不了图。
