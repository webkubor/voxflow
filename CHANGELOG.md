...
一次真实的端到端发行（两首歌从生成到上架）把整条链上的问题都踩了出来，
这个版本是那次踩坑的产物：三个新能力、六个修复，以及被驳回一次换来的封面规矩。

### 🎬 宣推短视频独立成页

reel-kit 的接入后端早就齐了（四个端点、五个模板、已产出 4 个视频），
前端却只是「全网发行」页里的一个弹窗 —— **做完的能力没人找得到**。

- 左侧挑歌（按有无音频过滤）、中间调参数、下方产出可直接播放下载
- 模板列表取自后端实测的 `reel templates`，装了新模板自动出现
- 字幕留空自动从歌词精选；纯音乐建议手写

### 🔏 原创存证：把「这首歌是我设计的」变成可验证的证据

AI 音乐能否受著作权保护，法律上看有没有**独创性智力投入**。
而这份证据 voxflow 一直存着：prompt、tags、两版里选了哪版、时间线、调用流水。

- `core/attest.py`：留痕导出、内容哈希、**Merkle 批量锚定**
  （逐首上链 = N 份 gas；Merkle 只上链一个 root，一笔锚定整批）
- 存证页：留痕可视化 + **证书图导出**（canvas 现画 1200×1600 PNG，
  刻意不依赖任何绘图库 —— 证书要能长期复现）
- `anchor_payload()` 只产出交易 data，**不碰私钥、不发交易**
- 15 项自检，含「篡改叶子后证明失效」

### 📀 专辑：发行后不可增删

汽水原话「一次性添加所有歌曲，发行后不可增删」。本地不挡的话，
会一路备料到提交那刻才发现加不进去，前面的封面和发行名全白做。

### 🐛 修复

- **封面被平台驳回**：出图时自拼 prompt 绕过了 `build_prompt` 里
  「不得出现文字」的禁令，还把专辑简介（营销文案）喂给了模型，
  结果画成了带标语角标的短视频海报。现统一走 `build_prompt`，
  画面主体改为**音乐元素本身**，并从专辑名剥掉品类词
- **汽水同步三个问题**：① 列表不是标准 `<tr>/<td>`，按 tr/td 取行永远是空，
  却报「一条作品都没读到」看着像账号不对；② 不传 song_id 导致重复记录，
  同一首歌两行状态各说各话；③ **「代理发行中」被当成审核中** ——
  它实际是已上线，7 首歌因此在台账里一直显示审核中、收益口径少算
- `set_platform_status` 按 song_id 查却没取 status → `IndexError`，
  而这正是所有同步脚本走的路径
- `upsert_album` 用 `INSERT … ON CONFLICT`，局部更新时 INSERT 那半边缺
  NOT NULL 列，SQLite 在冲突检测前就炸 —— 专辑封面出了两张都写不进库
- 删掉 `web/app.py` 里 190 行**完全重复的定义**（`_run_cover_task` 等），
  后者覆盖前者，改前一份完全不生效 —— 它是当天两个 bug 的共同根因
- ego space 的 ownership 有三种值，`handOff` 之后是 `agentDelegatedToUser`，
  只判 `user` 会漏掉

### 🎨 体验

- **导航层级**：一级和二级原来各画一条底线、中间零间距，两条线贴在一起
  读不出层级。改为一级保持标签页、二级降为胶囊按钮组，中间留间距
- **发行页从约十屏压到 1 屏**：教学内容与前置检查默认收起
  （有阻塞项时自动展开），三个短字段并排
- 「自动化发布」改名「导入音频」并**收进卡片墙**：导入外部音频只是挑歌时的
  一个小动作（别人给了个链接），不值得单独占一个 tab。
  路由 `#/intake` 保留，老链接不断
- tab 顺序改成一首歌真实走过的路：**挑歌 → 专辑 → 看进度** ——
  卡片墙·选歌 → 专辑 → 发歌记录 → 全网发行 → 运营台 → 宣推短视频 → 原创存证。
  任何歌都要进专辑才能发，所以专辑紧跟着挑歌

### 📚 文档

- `docs/README.md` **文档索引**（此前十几份文档没有任何入口）
- `DATA.md` 与现实同步：台账早已在 SQLite，原文却写着「没有数据库」。
  **过时的架构描述比没有文档更糟，它让人从错误前提开始推理**
- `MUSIC_PUBLISH_SOP` 补「封面红线」：边界是**引流信息**不是文字 ——
  专辑名和艺人名本来就是唱片封面的标准元素
...
平台发行本来就是按**辑**走的，但之前每首歌各出一张封面 —— 一张 1 积分，
十首歌就是十张的钱，而平台上架时它们本来要共用一张。把组辑这一步提到发行**之前**，
封面成本从 N 张压到 1 张，曲序和归属也在交平台前就定死，不用等平台侧再调。

- 一首歌同时只能在一张辑里；曲序按加入顺序
- 封面挂在**专辑**上，整辑共用（`sync_album_cover()`）
- 专辑按平台隔离（`album_key(platform, album_id)`），同一批歌可以在不同平台组不同的辑
- 移出歌曲后 `track_count` 同步；**专辑操作不打回歌曲状态**（组辑不是退回草稿）
- 新增 Albums 页：左列已有的辑，右边是选中那张的详情 + 把歌加进去
- `tests/test_pipeline_album.py` 12 项自检

### 🎯 发行前改名：交到平台之前随便改，之后锁死

名字定错了要能改，但**交到平台后台（uploaded 及以后）再改，本地就和平台对不上了**
—— 平台那边已经按旧名进审核队列。所以把可改窗口划在 uploaded 之前。

- `rename_release()`：uploaded 及以后一律拒绝
- 改名照样要全局唯一（跟新建时同一把尺子）
- **拒绝时名字不动**，不留半改状态
- `tests/test_pipeline_rename.py` 6 项自检

### 🐛 AI 风格标签时好时坏：不是模型不会答，是被截断了

报错是「没能生成风格标签」，看起来像模型能力问题，主题其实写得挺好。
实测抓到真相：失败那几次 `completion_tokens` **正好等于 200**（打满上限），
成功那次只有 33。会思考的模型（deepseek-v4-flash）要先花几百 token 推理才吐正文，
卡在 200 时思考还没完就被截断，`content` 返回空字符串。

**上限撞满 = 被截断，不是模型不会答。** `max_tokens` 200 → 800。

### ⚙️ 汽水音乐：状态同步 + 发布脚本迁 ego-browser

- `scripts/sync_qishui.py`：从汽水后台同步作品状态回本地台账
- `scripts/publish_qishui_ego.py`：发布脚本从 `browser-harness` 迁到 `ego-browser`。
  原因是 harness 依赖日常 Chrome 那个**默认关着**的「允许远程调试」开关，
  关着时报 `Runtime.evaluate timed out` —— 完全看不出是授权问题，而「自动化发布」页
  还把 harness 标成「✓ 已装」（只验了文件存在）。前置检查全绿却发不出去，最难排查。
  ego-browser 自带隔离浏览器、继承日常登录态，不需要任何开关。
- 仍然**停在提交前**：自动化的价值在填表那 10 分钟，不在最后点提交那 1 秒。
  提交不可逆（进审核队列要改得走撤回），AI 生成标识也必须人来勾（2026 新规漏标会下架封号）。
- `core/suno_api.py`：hCaptcha 解题走 ego 隔离浏览器；新增 `upload_audio()`

### 📚 docs 有索引了

`docs/` 下十几个文件此前没有任何索引，每次都要靠人提醒「项目里有 SOP」，
agent 更是每次重新摸一遍。新增 `docs/README.md`，带状态标记
（✅ 当前有效 · ⚠️ 部分过时 · 📋 计划/待评审 —— 计划不代表已实现）。
另补 `docs/SOP_从小白到唱一首歌.md`。

> **改 Suno 云端标题的端点别再盲试了**：`/api/gen/{id}/` 的 POST 和 PATCH 都是 404，
> 要补就用 req 抓一次网页端改名的真实请求。现成可用的是 Rust CLI
> `suno set <clip_id> --title "新名字"`（实测有效、免费）。而且发行用的**不是**它 ——
> 平台上架名存在本地 `tracks.release_title`，走 `pipeline.submit_release()`，
> 跟 Suno 侧标题互不影响。
